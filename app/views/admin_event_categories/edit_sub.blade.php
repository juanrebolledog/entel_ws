...
<section>
    <header>
        <h3>
            <?= link_to(action('AdminEventCategoriesController@index'), 'Categoría de Eventos'); ?> &raquo;  {{ $category->nombre }} &raquo; Editar
        </h3>
    </header>
    <?= Form::model($category, array('url' => action('AdminEventCategoriesController@update_sub', $category->id), 'method' => 'put', 'files' => true)); ?>
    <fieldset>
