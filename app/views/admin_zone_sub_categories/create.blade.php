...
<section>
    <header>
        <h3>
            {{ link_to(action('AdminZoneCategoriesController@index'), 'Categorías de Zonas') }} &raquo; {{ link_to(action('AdminZoneCategoriesController@show', $category->id), $category->nombre) }} &raquo; {{ 'Crear Sub Categoría' }}
        </h3>
    </header>
    {{ Form::model($sub_category, array('url' => action('AdminZoneSubCategoriesController@store', $category->id), 'files' => true)) }}
        <fieldset>
