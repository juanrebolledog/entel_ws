...
<section>
    <header>
        <h3>
            <?= link_to(action('AdminBenefitCategoriesController@index'), 'Categorías de Beneficios'); ?> &raquo; <?= link_to(action('AdminBenefitCategoriesController@show', $category->id), $category->nombre); ?> &raquo; Editar
        </h3>
    </header>
    <?= Form::model($category, array('url' => action('AdminBenefitCategoriesController@update', $category->id), 'method' => 'put', 'files' => true)); ?>
        <fieldset>
